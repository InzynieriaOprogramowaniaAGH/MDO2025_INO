...
Utworzyłem swoją gałąź SM417733
...
Następnie pobrałem obrazy docker: hello-world, busybox, ubuntu lub fedora, mysql według schematu:
...
Zanim przystąpiłem do pracy na maszynie wirtualnej, poszukałem oprogramowani, które dysponuje otwartą licencją i ma możliwość zbudowania i uruchomienia testów. Mój wybór padł na ```redis```.
...
Uruchomiłem kontener bazowy w ```node```, do którego podłączyłem woluminy wejściowy i wyjściowy:
```
...
Ay sklonować repozytorium Redis'a do woluminu wejściowego, muszę najpierw sklonować je na hoście i następnie skopiować je do woluminu:
```
