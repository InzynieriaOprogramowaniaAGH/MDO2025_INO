...
Opis zmian:
W pliku kickstart dodano sekcję %post, która po zakończeniu instalacji systemu automatycznie pobiera z serwera Jenkins pakiet .deb z aplikacją Irssi. Następnie pakiet jest ręcznie rozpakowywany za pomocą narzędzi ar i tar (dostarczonych dzięki dołączeniu pakietu binutils w sekcji %packages), a zawartość jest kopiowana do odpowiednich katalogów systemowych (/usr/). Aby program Irssi uruchamiał się automatycznie po zalogowaniu użytkownika psocala, do jego pliku .bash_profile dodawane jest polecenie uruchamiające /usr/bin/irssi. Po zakończeniu tych operacji czyszczone są tymczasowe pliki, a system zostaje automatycznie zrestartowany.
...
# Rozpakowywanie .deb
ar x irssi.deb
tar -xf data.tar.*  # może być .gz lub .xz

# Kopiowanie binariów do odpowiednich katalogów 
cp -r * /
