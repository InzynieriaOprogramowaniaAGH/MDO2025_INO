# Sprawozdanie 1

# Lab 1

### 1. Klonowanie repozytorium przez HTTPS

 ![Klonowanie repozytorium przez HTTPS](Lab_1/screeny/4.png)

### 2. Wygenerowanie klucza SSH 

 ![Wygenerowanie klucza SSH](Lab_1/screeny/3.png)

### 3. Sklonowanie repozytorium przez SHH 

 ![Sklonowanie repozytorium przez SHH](Lab_1/screeny/1.png)

### 4. Stworzenie git hook wymuszajacy format commit message

![Stworzenie git hook wymuszajacy format commit message](Lab_1/screeny/hook.png)

### 5. Dowod na dzialanie hooka

![Dowod na dzialanie hooka](Lab_1/screeny/6.png)

![Dowod na dzialanie hooka](Lab_1/screeny/5.png)

### 6. Dodanie przykladowego pliku tekstowego  

![Dodanie](Lab_1/screeny/2.png)

# Lab 2

### 1. Dodanie uzytkownika do grupy Docker i pobranie obrazu hello world

![1](Lab_2/screeny/1.png)

### 2. Pobranie pozostalych obrazow

![2](Lab_2/screeny/2.png)

### 3. Uruchominie kontenera BusyBox i sprawdzenie jego wersji

![3](Lab_2/screeny/3.png)

### 4. Uruchomienie Fedory w kontenerze

![5](Lab_2/screeny/5.png)

### 5. Pokazanie PID1 po zainstalowaniu procps

![6](Lab_2/screeny/6.png)

### 6. Aktualizacja pakietow

![7](Lab_2/screeny/7.png)

### 7. Przygotowanie Docerfile, ktory bedzize tworzyl kontener z fedora, instalowal gita i clonowal 
### repozytorium

![8](Lab_2/screeny/8.png)

### 8. Budowa obrazu 

![9](Lab_2/screeny/9.png)

### 9. Usuniecie wszystkich obrazow

![10](Lab_2/screeny/10.png)

# Lab 3

### 1. Sklonowanie repozytorium OpenCV 

![1](Lab_3/screeny/1.png)

### 2. Zainstalowanie potrzebnych narzedzi do builda

![2](Lab_3/screeny/2.png)

### 3. Proces kompilacji zakonczony sukcesem

![3](Lab_3/screeny/3.png)

### 4. Uruchumienie testu jednostkowego zakonczonego bledem segmentation fault 
### (z jakiegos powodu sie uparlem zeby wykonal sie ten test do konca ale ogolem te krotsze testy dziala)

![4](Lab_3/screeny/4.png)

### 5. Pobranie obrazu ubuntu i uruchomienie kontere interaktywnie, zainstalowanie git, cmake oraz build-essentials

![5](Lab_3/screeny/5.png)

### 6. Sklonowanie repozytorium openCV w kontenerze

![6](Lab_3/screeny/6.png)

### 7. Aktualizacja pakietow w kontenerze

![7](Lab_3/screeny/7.png)

### 8. Uruchomienie konfiguracji projektu OpenCV przy pomocy cmake w katalogu build

![8](Lab_3/screeny/8.png)

### 9. Uruchumiene procesu kompilacji przez make

![9](Lab_3/screeny/9.png)

### 10. Test jednostkowy przerwany przeze mnie 
### (krotsze testy dzialaja screeny w lab 4)

![10](Lab_3/screeny/10.png)

### 11. Plik Dockerfile.build, ktory bazuje na ubuntu, instaluje narzedzia, 
### klonuje repozytorium, tworzy katalog build i wykonuje budowe projektu

![11](Lab_3/screeny/11.png)

### 12. Zbudowanie Dockera OpenCV-builder na podstawie Dockerfile.build

![12](Lab_3/screeny/12.png)

### 13. Dockerfile.test bazuje na obrazie OpenCV-builder i uruchamia test opencv_test_core z katalogu build/bin

![13](Lab_3/screeny/13.png)

### 14. Zbudowanie obrazu Dockera OpenCV-builder na podstawie Dockerfile.test

![14](Lab_3/screeny/14.png)

### 15. Budowa obrazu OpenCV-builder zakonczona pomyslnie

![15](Lab_3/screeny/15.png)

# Lab 4

### 1. Przygotowywanie woluminow

![1](Lab_4/screeny/1.png)

### 2. Uruchomienie kontenera bazowego i sklonowanie repozytorium na wolumin wejsciowy za pomoca kontenera alpine

![2](Lab_4/screeny/2.png)

### 3. Wykonanie build w kontenerze

![3](Lab_4/screeny/3.png)

### 4. Uruchomienie testow jednostkowych

![4](Lab_4/screeny/4.png)

### 5. Sprawdzanie plikow wynikowych

![5](Lab_4/screeny/5.png)

### 6. Tworzenie sieci mostkowej (detykowanej)

![6](Lab_4/screeny/6.png)

### 7. Uruchomienie klienta iperf3

![7](Lab_4/screeny/7.png)

### 8. Wyniki testu przepustowosci z uzyciem iperf3

![8](Lab_4/screeny/8.png)

### 9. Uruchomienie kontenera Jenkins Docker-in-Docker pelniacego role hosta Jenkins

![9](Lab_4/screeny/9.png)

### 10. Uruchomienie kontenera Jenkins na dedykowanej sieci

![10](Lab_4/screeny/10.png) 

### 11. Dzialajacy Jenkins

![11](Lab_4/screeny/11.png)
