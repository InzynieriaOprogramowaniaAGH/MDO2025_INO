...
1.  W panelu **Manage Jenkins > Configure Global Security** włączono opcję **Matrix-based security**.
2.  Dla grupy **Authenticated Users** (użytkownicy zalogowani) przyznano pełne uprawnienia do wszystkich operacji (Build, Read, Configure itp.).
3.  Dla użytkownika **Anonymous** (użytkownicy niezalogowani) odebrano kluczowe uprawnienia, w tym `Build > Build`, `Job > Read` (aby nie widzieli szczegółów zadań) oraz `Run > Replay`. Pozostawiono jedynie ogólne uprawnienie `Overall > Read`, aby strona główna była widoczna.
...
> **📷 Zrzut ekranu 3:** Zrzut ekranu panelu konfiguracyjnego `Matrix-based security` w Jenkinsie, na którym widać tabelę z uprawnieniami dla grup `Anonymous` i `Authenticated Users`.
...
Po zakończeniu całej konfiguracji, gotowa maszyna wirtualna została wyeksportowana do pliku w formacie **OVA (Open Virtualization Appliance)**. Plik ten stanowi kompletną, przenośną kopię zapasową skonfigurowanego środowiska.

> **📷 Zrzut ekranu 4:** Umieść zrzut okna dialogowego z programu VirtualBox (lub innego) pokazujący proces eksportowania maszyny wirtualnej do pliku `.ova`.

---
---

## 2. Implementacja Pipeline CI/CD
...
> **📷 Zrzut ekranu 5:** Fragment logu konsoli z Jenkinsa dla etapu `Build`, na którym widać końcowe linie działania polecenia `make install` oraz komunikat `>>> BUILD END`.
...
* **Użycie "buildera" jako obrazu bazowego:** Jako podstawę do uruchomienia testów wykorzystano obraz `my-httpd-built:latest`, który został stworzony w poprzednim kroku poprzez `docker commit`. Jest to logiczny wybór, ponieważ zawiera on dokładnie te skompilowane pliki binarne, które mają zostać przetestowane. Gwarantuje to spójność między tym, co zostało zbudowane, a tym, co jest testowane.
...
> **📷 Zrzut ekranu 6:** Widok logów konsoli Jenkinsa dla etapu `Test`. Powinien być widoczny wynik działania `pytest` z listą testów i podsumowaniem, np. `======== 12 passed in 3.45s ========`.

---

### 2.3. Etapy `Deploy` i `Publish`

Końcowe etapy pipeline'u odpowiadają za przygotowanie finalnego artefaktu oraz jego dystrybucję.

* **Deploy:** W tym kroku tworzony jest minimalny obraz `my-httpd:latest` przeznaczony do uruchomienia produkcyjnego. Kopiowane są do niego wyłącznie niezbędne, skompilowane pliki z obrazu `my-httpd-built`, bez narzędzi deweloperskich. Następnie uruchamiany jest test typu "smoke test", który sprawdza, czy serwer poprawnie startuje i odpowiada na zapytania sieciowe.
* **Publish:** Po pomyślnym wdrożeniu i teście, artefakty są publikowane. Obraz Docker `my-httpd:latest` jest tagowany i wysyłany do publicznego repozytorium **Docker Hub**. Dodatkowo, spakowane archiwum `.tar.gz` z plikami instalacyjnymi serwera jest archiwizowane w Jenkinsie, skąd może być pobrane ręcznie lub przez inne procesy.

> **📷 Zrzut ekranu 7:** Umieść zrzut ekranu przedstawiający Twoje repozytorium `my-httpd` na stronie Docker Hub, z widocznymi tagami `latest` i numerem ostatniego buildu.

> **📷 Zrzut ekranu 8:** Zrzut ekranu strony builda w Jenkinsie, na której w sekcji **Build Artifacts** widoczny jest plik `my-httpd-install.tar.gz` gotowy do pobrania.
...
Wnioski dotyczące dystrybucji
Czy program powinien być pakowany?
Tak, ale w naszym przypadku najlepszym sposobem jest obraz Docker, bo:
Zapewnia środowisko runtime niezależne od systemu hosta.
Można go łatwo uruchomić na dowolnej maszynie lub w Kubernetes.
Nie wymaga instalowania zależności buildowych na docelowej maszynie.
Co powinien zawierać pakiet / obraz?
Tylko runtime:
Skompilowane binaria (httpd/install/)
Konfiguracja i ewentualne statyczne pliki (jeśli istnieją)
Nie:
Repozytoria Git
Pliki builda, logi, narzędzia developerskie
Zależności potrzebne tylko do kompilacji
Czy obraz powinien zawierać repo, logi i artefakty builda?
Nie. Obraz produkcyjny musi być lekki i zawierać tylko to, co potrzebne do uruchomienia serwera.